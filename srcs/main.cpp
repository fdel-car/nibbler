		...
		if (is_number(av[1]) && is_number(av[2])) {
			if (strlen(av[1]) < 9 && strlen(av[2]) < 9) {
				...
				if (config.width >= 20 && config.width <= 40 && config.height >= 20 &&
					...
					return printError("Invalid width and height. They should be between 20 and 40.");
			}
			else
				return printError("Width or height overflow. They should be between 20 and 40.");
		}
		else
			return printError("Invalid width and height. They should be positives numbers between 20 and 40.");
	}
	else
